...
WORDS_FILE = OUTPUT_DIR / "missing_bauernspruch_words.json"
CONTROL_FILE = OUTPUT_DIR / "sn_bauernspruch_control.json"
PROGRESS_FILE = OUTPUT_DIR / "sn_bauernspruch_progress.json"
PATCH_FILE = OUTPUT_DIR / "sprachnudel_bauernspruch_patch.jsonl"
SPEED_CONFIGS = {
...
def _load_word_map() -> dict:
    if not WORDS_FILE.exists():
        print(f"Wortliste nicht gefunden: {WORDS_FILE}")
        return {}
...
def main():
    ...
    patch_entries = None
    speed = "normal"
    print("=== Bauernspruch Wortliste Scrape (VPN/PARALLEL) ===")

    word_map = _load_word_map()
    if not word_map:
        return

    all_words = [v["wort"] for v in word_map.values()]
    word_categories = {v["wort"].casefold(): v["kategorie"] for v in word_map.values()}
    print(f"Zu scrapen: {len(all_words)} Woerter in {len(set(word_categories.values()))} Kategorien")
    ...
    if not remaining:
        print("Nichts zu tun!")
        _write_control("done", "Alle Bauernspruch-Woerter verarbeitet")
        return
    ...
    _write_control("running", f"Bauernspruch: {len(unique_words)} Woerter, {max_workers} parallel", speed=speed)
                    ...
                    try:
                        ...
                        if outcome == "ok" and entry:
                            ...
                            if key in word_categories:
                                entry["kategorie"] = word_categories[key]
